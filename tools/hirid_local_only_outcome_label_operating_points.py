...
OUT_JSON = OUT_DIR / "hirid_local_only_outcome_label_operating_points.json"
OUT_MD = OUT_DIR / "hirid_local_only_outcome_label_operating_points.md"
AUDIT_JSON = AUDIT_DIR / "hirid_local_only_outcome_label_operating_points_audit.json"
WORK_DB = WORK_DIR / "hirid_private_outcome_label_work.sqlite"
...
def load_mapping() -> dict[int, list[str]]:
    ...
    if not isinstance(vm, dict):
        raise SystemExit("Mapping must be dictionary-shaped before outcome-label pass.")
...
def find_reference_tarball() -> Path:
    ...
    if not candidates:
        raise SystemExit("Could not find reference/general table tarball.")
    return candidates[0]
...
def outcome_label(raw_status: str) -> str | None:
    text = str(raw_status).strip().lower()
    if text in {"dead", "deceased", "death", "died", "1", "true"}:
        return "event"
    if text in {"alive", "survived", "survivor", "0", "false"}:
        return "non_event"
    return None
...
def init_db(conn: sqlite3.Connection):
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS patient_outcome (
            pid_hash TEXT PRIMARY KEY,
...
def load_patient_outcomes(conn: sqlite3.Connection, ref_tar: Path) -> dict:
    ...
    with tarfile.open(ref_tar, "r:gz") as tf:
        members = [
            ...
            if m.isfile() and m.name.lower().endswith(".csv") and "general_table" in m.name.lower()
        ]

        if not members:
            raise SystemExit("No general_table.csv found inside reference tarball.")

        m = members[0]
        f = tf.extractfile(m)
        if f is None:
            raise SystemExit("Could not extract general_table.csv.")
        ...
        for row in reader:
            ...
            status = row_get(row, ["discharge_status", "dischargestatus", "status"])

            label = outcome_label(status)
            if not pid or label is None:
                skipped += 1
                continue
            ...
            existing = conn.execute(
                "SELECT outcome FROM patient_outcome WHERE pid_hash = ?",
                (ph,)
            ).fetchone()
            ...
            conn.execute(
                "INSERT INTO patient_outcome(pid_hash, outcome) VALUES (?, ?)",
                (ph, label)
            )
    ...
    return {
        "reference_tarball": str(ref_tar),
        "loaded_patients": loaded,
        "skipped_rows": skipped,
        "duplicate_patient_rows": duplicate,
...
def main():
    ...
    print("Reference/general tarball:", ref_tar)
    ...
    outcome_load = load_patient_outcomes(conn, ref_tar)
    print("Loaded outcome labels:", outcome_load)
    ...
    print("")
    print("Building aggregate score bins...")
    ...
    labeled_patients = conn.execute("SELECT COUNT(*) FROM patient_outcome").fetchone()[0]
    event_patients = conn.execute("SELECT COUNT(*) FROM patient_outcome WHERE outcome = 'event'").fetchone()[0]
    non_event_patients = conn.execute("SELECT COUNT(*) FROM patient_outcome WHERE outcome = 'non_event'").fetchone()[0]

    patients_with_any_signal = conn.execute("""
        SELECT COUNT(DISTINCT sb.pid_hash)
        FROM score_bin sb
        JOIN patient_outcome po ON po.pid_hash = sb.pid_hash
        WHERE sb.review_score > 0
    """).fetchone()[0]

    threshold_results = []

    for t in THRESHOLDS:
        event_reviewed = conn.execute("""
            SELECT COUNT(DISTINCT po.pid_hash)
            FROM patient_outcome po
            JOIN score_bin sb ON sb.pid_hash = po.pid_hash
            WHERE po.outcome = 'event'
              AND sb.review_score >= ?
        """, (t,)).fetchone()[0]

        non_event_reviewed = conn.execute("""
            SELECT COUNT(DISTINCT po.pid_hash)
            FROM patient_outcome po
            JOIN score_bin sb ON sb.pid_hash = po.pid_hash
            WHERE po.outcome = 'non_event'
              AND sb.review_score >= ?
        """, (t,)).fetchone()[0]
        ...
        event_capture_proxy = (event_reviewed / event_patients * 100.0) if event_patients else None
        non_event_review_proxy = (non_event_reviewed / non_event_patients * 100.0) if non_event_patients else None
        labeled_review_rate = (all_reviewed / labeled_patients * 100.0) if labeled_patients else None
        event_mix_among_reviewed = (event_reviewed / all_reviewed * 100.0) if all_reviewed else None

        threshold_results.append({
            "threshold": t,
            "positive_review_bins": positive_bins,
            "labeled_patients_reviewed": all_reviewed,
            "event_patients_reviewed": event_reviewed,
            "non_event_patients_reviewed": non_event_reviewed,
            "event_capture_proxy_percent": round(event_capture_proxy, 4) if event_capture_proxy is not None else None,
            "non_event_review_proxy_percent": round(non_event_review_proxy, 4) if non_event_review_proxy is not None else None,
            "labeled_patient_review_rate_percent": round(labeled_review_rate, 4) if labeled_review_rate is not None else None,
            "event_mix_among_reviewed_percent": round(event_mix_among_reviewed, 4) if event_mix_among_reviewed is not None else None,
        })

    output = {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "status": "local_only_outcome_label_operating_point_pass_not_public_validation",
        ...
        "method_note": (
            "Private aggregate-only outcome-proxy pass using discharge_status as event/non-event label. "
            "This is not clinical validation and does not establish prospective performance. "
            "No patient-level outputs, timestamps, raw rows, or identifiers are exported."
        ),
        "label_definition": {
            "event": "discharge_status interpreted as dead/deceased/death/died",
            "non_event": "discharge_status interpreted as alive/survived",
            "label_type": "outcome proxy only"
        },
        "overall_counts": {
            ...
            "labeled_patients_total": labeled_patients,
            "event_patients_total": event_patients,
            "non_event_patients_total": non_event_patients,
            "labeled_patients_with_any_signal": patients_with_any_signal,
        },
        "outcome_load": outcome_load,
    ...
    AUDIT_JSON.write_text(json.dumps(output, indent=2), encoding="utf-8")

    lines = [
        "# HiRID Local-Only Outcome-Label Operating-Point Pass",
        "",
        f"Timestamp UTC: {output['timestamp_utc']}",
        "",
        "**Status:** Local-only outcome-label operating-point pass. This is **not public validation**.",
        "",
        ...
        "## Label Definition",
        "",
        "- Event label: discharge_status interpreted as dead/deceased/death/died.",
        "- Non-event label: discharge_status interpreted as alive/survived.",
        "- Label type: outcome proxy only.",
        "",
        ...
        f"- Labeled patients total: {labeled_patients:,}",
        f"- Event patients total: {event_patients:,}",
        f"- Non-event patients total: {non_event_patients:,}",
        f"- Labeled patients with any signal: {patients_with_any_signal:,}",
        "",
        "## Threshold Outcome-Proxy Summary",
        "",
        "| Threshold | Positive bins | Labeled patients reviewed | Event patients reviewed | Non-event patients reviewed | Event capture proxy | Non-event review proxy | Overall labeled review rate | Event mix among reviewed |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for r in threshold_results:
        lines.append(
            ...
            f"{r['labeled_patient_review_rate_percent']}% | "
    ...
    lines.extend([
        ...
        "## Guardrail",
        "",
        "This is local-only aggregate outcome-proxy behavior. Do not publish this as HiRID validation.",
        "",
        "This pass does **not** establish clinical validation, prospective validation, final HiRID performance, diagnosis, treatment direction, or independent escalation.",
        "",
    ...
    print("")
    print("DONE — local-only outcome-label operating-point pass complete.")
