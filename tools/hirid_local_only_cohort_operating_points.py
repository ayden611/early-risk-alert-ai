...
OUT_JSON = OUT_DIR / "hirid_local_only_cohort_operating_points.json"
OUT_MD = OUT_DIR / "hirid_local_only_cohort_operating_points.md"
WORK_DB = WORK_DIR / "hirid_private_cohort_work.sqlite"
...
def cohort_from_hash(pid_hash: str) -> str:
    n = int(pid_hash[:8], 16) % 3
    return ["cohort_a", "cohort_b", "cohort_c"][n]
...
def load_mapping() -> dict[int, list[str]]:
    ...
    if not isinstance(vm, dict):
        raise SystemExit("Mapping must be dictionary-shaped before cohort pass.")
...
def init_db(conn: sqlite3.Connection):
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS vital_bin (
            ...
            vital TEXT NOT NULL,
            score REAL NOT NULL,
            PRIMARY KEY (pid_hash, cohort, hour_bin, vital)
        )
    """)
    conn.commit()

def upsert_batch(conn: sqlite3.Connection, rows: list[tuple[str, str, int, str, float]]):
    if not rows:
        return
    conn.executemany("""
        INSERT INTO vital_bin(pid_hash, cohort, hour_bin, vital, score)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(pid_hash, cohort, hour_bin, vital)
...
def main():
    ...
    print("Streaming rows. Cohort outputs are aggregate only. No raw rows printed.")
    print("")
    ...
    cohort_match_counts = defaultdict(int)
    cohort_score_positive_counts = defaultdict(int)
        ...
        for idx, m in enumerate(members, start=1):
            ...
            for row in reader:
                ...
                for vital in id_to_vitals[vid]:
                    ...
                    matched_rows += 1
                    member_matches += 1
                    cohort_match_counts[cohort] += 1
                    vital_match_counts[vital] += 1

                    score = score_value(vital, x)
                    if score > 0:
                        scored_rows += 1
                        cohort_score_positive_counts[cohort] += 1
                        batch.append((ph, cohort, hour_bin, vital, score))
    ...
    print("")
    print("Building aggregate cohort bins...")

    conn.execute("DROP TABLE IF EXISTS score_bin")
    conn.execute("""
        CREATE TABLE score_bin AS
        SELECT cohort, pid_hash, hour_bin, SUM(score) AS review_score
        FROM vital_bin
        GROUP BY cohort, pid_hash, hour_bin
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_score_bin_cohort_score ON score_bin(cohort, review_score)")
    conn.commit()
    ...
    for cohort in cohorts:
        total_bins = conn.execute(
            "SELECT COUNT(*) FROM score_bin WHERE cohort = ?",
            (cohort,)
        ).fetchone()[0]

        any_signal_bins = conn.execute(
            "SELECT COUNT(*) FROM score_bin WHERE cohort = ? AND review_score > 0",
            (cohort,)
        ).fetchone()[0]

        unique_patients_any_signal = conn.execute(
            "SELECT COUNT(DISTINCT pid_hash) FROM score_bin WHERE cohort = ? AND review_score > 0",
            (cohort,)
        ).fetchone()[0]

        threshold_results = []
        previous_positive_bins = None
        monotonic_ok = True

        for t in THRESHOLDS:
            positive_bins = conn.execute(
                "SELECT COUNT(*) FROM score_bin WHERE cohort = ? AND review_score >= ?",
                (cohort, t)
            ).fetchone()[0]

            positive_patients = conn.execute(
                "SELECT COUNT(DISTINCT pid_hash) FROM score_bin WHERE cohort = ? AND review_score >= ?",
                (cohort, t)
            ).fetchone()[0]

            avg_score = conn.execute(
                "SELECT AVG(review_score) FROM score_bin WHERE cohort = ? AND review_score >= ?",
                (cohort, t)
            ).fetchone()[0]

            max_score = conn.execute(
                "SELECT MAX(review_score) FROM score_bin WHERE cohort = ? AND review_score >= ?",
                (cohort, t)
            ).fetchone()[0]
            ...
            review_rate = (positive_bins / total_bins * 100.0) if total_bins else None
            reduction = (
                (1.0 - (positive_bins / any_signal_bins)) * 100.0
                if any_signal_bins else None
            )

            if previous_positive_bins is not None and positive_bins > previous_positive_bins:
                monotonic_ok = False
            previous_positive_bins = positive_bins

            threshold_results.append({
                "threshold": t,
                "positive_review_bins": positive_bins,
                "positive_review_rate_among_scored_bins_percent": round(review_rate, 4) if review_rate is not None else None,
                ...
                "estimated_reduction_vs_any_single_signal_bins_percent": round(reduction, 4) if reduction is not None else None,
        ...
        cohort_results.append({
            ...
            "matched_vital_rows_total": cohort_match_counts.get(cohort, 0),
            "scored_signal_rows_total": cohort_score_positive_counts.get(cohort, 0),
            "monotonic_threshold_behavior": monotonic_ok,
            "threshold_results": threshold_results,
        })

    output = {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "status": "local_only_cohort_operating_point_pass_not_public_validation",
        ...
        "method_note": (
            "Private deterministic cohort split using local-only pseudonymous patient hash. "
    ...
    lines = [
        "# HiRID Local-Only Cohort Operating-Point Pass",
        "",
        f"Timestamp UTC: {output['timestamp_utc']}",
        "",
        "**Status:** Local-only cohort operating-point pass. This is **not public validation**.",
        "",
    ...
    for cohort in cohort_results:
        ...
        lines.append("")
        lines.append(f"- Total scored patient-hour bins: {cohort['total_scored_patient_hour_bins']:,}")
        lines.append(f"- Any-signal bins: {cohort['any_signal_bins']:,}")
        lines.append(f"- Unique patients with any signal: {cohort['unique_patients_any_signal']:,}")
        lines.append(f"- Monotonic threshold behavior: {cohort['monotonic_threshold_behavior']}")
        lines.append("")
        lines.append("| Threshold | Positive review bins | Positive review rate | Unique patients | Avg score | Max score | Est. reduction vs any-signal bins |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|")

        for r in cohort["threshold_results"]:
            lines.append(
                f"| t={r['threshold']} | "
    ...
    lines.extend([
        "## Guardrail",
        "",
        "This is local-only aggregate cohort behavior. Do not publish this as HiRID validation.",
        "",
    ...
    print("")
    print("DONE — local-only cohort operating-point pass complete.")
