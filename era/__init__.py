...
<head>
  <meta charset="utf-8">
  <title>AI Hospital Command Center</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
    :root{
      --bg:#040a12;
    ...
    *{box-sizing:border-box}
    body{
      ...
      font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;
      background:
        radial-gradient(circle at top left, rgba(91,212,255,.08), transparent 20%),
        radial-gradient(circle at 84% 10%, rgba(122,162,255,.06), transparent 18%),
        linear-gradient(180deg,var(--bg),var(--bg2));
      color:var(--text);
      overflow-x:hidden;
    }
    ...
    .topbar{
      ...
      margin-bottom:18px;padding:18px 20px;border:1px solid var(--line);border-radius:22px;
      background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.015)),var(--panel);
      box-shadow:var(--shadow);
    }
    .title-k{font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:#8fd7ff}
    .title-h{margin-top:8px;font-size:42px;font-weight:1000;line-height:.95;letter-spacing:-.05em}
    .title-s{margin-top:10px;color:#c6d7ef;line-height:1.65;max-width:900px}
    .top-actions{display:flex;gap:10px;flex-wrap:wrap}
    .btn{
      display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:13px 16px;border-radius:14px;
      font-size:14px;font-weight:1000;border:1px solid transparent;text-decoration:none
    }
    ...
    .ticker-wrap{
      margin-bottom:18px;border:1px solid var(--line);border-radius:18px;overflow:hidden;
    ...
    .kpi .v{margin-top:10px;font-size:34px;font-weight:1000;line-height:1}
    .kpi .s{margin-top:8px;color:#c6d7ef;font-size:13px;line-height:1.5}
    .grid{display:grid;grid-template-columns:1.45fr .95fr;gap:18px}
    .wall{
      border:1px solid var(--line);border-radius:22px;padding:18px;
      background:
        radial-gradient(circle at top right, rgba(91,212,255,.06), transparent 30%),
    ...
    .panel-title{font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:#9fdcff}
    .panel-copy{color:#c6d7ef;line-height:1.65}
    ...
    .monitor-top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}
    ...
    .badge{
      display:inline-flex;align-items:center;justify-content:center;min-width:92px;padding:8px 10px;border-radius:999px;
    ...
    .screen{
      position:relative;height:220px;border-radius:18px;overflow:hidden;border:1px solid rgba(255,255,255,.06);background:#050b14;
    }
    .screen-grid{
    ...
    .wave-line.critical{stroke:#ff667d;color:#ff667d}
    .wave-line.high{stroke:#f4bd6a;color:#f4bd6a}
    .wave-line.stable{stroke:#38d39f;color:#38d39f}
    @keyframes ecgMove{from{stroke-dashoffset:0}to{stroke-dashoffset:-52}}
    .screen-readouts{
      position:absolute;left:12px;right:12px;bottom:12px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px;
    }
    .readout{border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:10px;background:rgba(255,255,255,.03);backdrop-filter:blur(4px)}
    .readout .rk{display:block;font-size:10px;letter-spacing:.10em;text-transform:uppercase;color:#9eb4d6;font-weight:900}
    .readout .rv{display:block;margin-top:6px;font-size:20px;line-height:1;font-weight:1000}
    .right-col{display:grid;gap:18px}
    .rail{
      border:1px solid var(--line);border-radius:22px;padding:18px;
      background:linear-gradient(180deg, rgba(255,255,255,.03), rgba(255,255,255,.015)),var(--panel2);
      box-shadow:var(--shadow);
    }
    ...
    .rail-v{margin-top:10px;font-size:32px;font-weight:1000;line-height:1}
    ...
    .feed-list{display:grid;gap:10px;margin-top:14px}
    .feed-item{padding:12px 14px;border:1px solid rgba(255,255,255,.08);border-radius:14px;background:rgba(255,255,255,.03)}
    .feed-item .n{font-size:14px;font-weight:1000;color:#eef4ff;display:flex;align-items:center;gap:10px}
    .feed-item .m{font-size:13px;color:#bdd0ea;margin-top:6px;line-height:1.5}
    .dot{width:9px;height:9px;border-radius:999px;background:#7aa2ff;display:inline-block;box-shadow:0 0 8px rgba(122,162,255,.45)}
    .dot.high{background:#f4bd6a;box-shadow:0 0 8px rgba(244,189,106,.35)}
    .dot.critical{background:#ff667d;box-shadow:0 0 8px rgba(255,102,125,.45)}
    .dot.stable{background:#38d39f;box-shadow:0 0 8px rgba(56,211,159,.35)}
    .proof{
    ...
    .proof-v{margin-top:10px;font-size:26px;font-weight:1000;line-height:1}
    .proof-s{margin-top:8px;color:#c6d7ef;font-size:13px;line-height:1.5}
...
<body>
  <div class="wrap">
    <div class="topbar">
      <div>
        <div class="title-k">AI Hospital Mission Control</div>
        <div class="title-h">Early Risk Alert AI · Live Predictive Monitoring Wall</div>
        <div class="title-s">
          Real-time patient simulation, animated ECG waveform monitors, risk scoring, alert hierarchy,
          live feed, and mission-control operating visibility from one flagship command center.
        </div>
      </div>
    ...
    <div class="ticker-wrap">
      <div class="ticker-head">
        ...
        <div class="ticker-live">Predictive Stream</div>
      </div>
      <div class="ticker-track">
        <div class="ticker-move" id="ticker">
          ...
          <div class="ticker-pill"><span class="dot high"></span> Mission control telemetry synced</div>
        </div>
      </div>
    </div>

    <div class="kpi-grid">
      <div class="kpi"><div class="k">Open Alerts</div><div class="v" id="k-open">0</div><div class="s">Active AI-flagged alerts under monitoring</div></div>
      <div class="kpi"><div class="k">Critical Alerts</div><div class="v" id="k-critical">0</div><div class="s">Highest urgency cases requiring immediate action</div></div>
      <div class="kpi"><div class="k">Avg Risk Score</div><div class="v" id="k-risk">0.0</div><div class="s">Average risk across flagged patients</div></div>
      <div class="kpi"><div class="k">Patients With Alerts</div><div class="v" id="k-patients">0</div><div class="s">Patients currently surfaced by AI monitoring</div></div>
      <div class="kpi"><div class="k">Events Last Hour</div><div class="v" id="k-events">0</div><div class="s">Clinical events simulated by the monitoring engine</div></div>
      <div class="kpi"><div class="k">Prediction Engine</div><div class="v" id="k-engine">LIVE</div><div class="s">Risk scoring, trend analysis, and alert escalation online</div></div>
    </div>
    ...
    <div class="grid">
      <div class="wall">
        <div class="panel-head">
          <div>
            <div class="panel-title">Waveform monitor wall</div>
            <div class="panel-copy">Four active patient monitors showing animated ECG waveforms, vitals, severity, and risk progression.</div>
          </div>
        </div>
        ...
        <div class="monitor-grid">
          <div class="monitor critical" id="card-m1">
            <div class="monitor-top">
              <div>
                ...
                <div class="monitor-name" id="m1-name">Patient 1042</div>
              </div>
          ...
          <div class="monitor high" id="card-m2">
            <div class="monitor-top">
              <div>
                ...
                <div class="monitor-name" id="m2-name">Patient 2188</div>
              </div>
            ...
            <div class="screen">
              ...
              <div class="screen-readouts">
                ...
                <div class="readout"><span class="rk">Risk</span><span class="rv" id="m2-risk">8.2</span></div>
              </div>
            </div>
          </div>
          ...
          <div class="monitor stable" id="card-m3">
            <div class="monitor-top">
              <div>
                <div class="monitor-k">ICU Bed C</div>
                <div class="monitor-name" id="m3-name">Patient 3045</div>
              </div>
          ...
          <div class="monitor high" id="card-m4">
            <div class="monitor-top">
              <div>
                <div class="monitor-k">ICU Bed D</div>
                <div class="monitor-name" id="m4-name">Patient 4172</div>
              </div>
            ...
            <div class="screen">
              ...
              <svg class="wave-svg" viewBox="0 0 1200 220" preserveAspectRatio="none">
                <path id="m4-wave" class="wave-line high"
                  d="M0 136 L40 136 L80 136 L120 136 L160 136
                     L200 136 L220 126 L240 142 L260 100 L280 162 L300 136
                     L340 136 L380 136 L420 136
                     L460 136 L480 126 L500 142 L520 100 L540 162 L560 136
                     L600 136 L640 136 L680 136
                     L720 136 L740 126 L760 142 L780 100 L800 162 L820 136
                     L860 136 L900 136 L940 136
                     L980 136 L1000 126 L1020 142 L1040 100 L1060 162 L1080 136
                     L1120 136 L1160 136 L1200 136" />
              </svg>
              <div class="screen-readouts">
        ...
        <div class="proof">
          <div class="proof-card"><div class="proof-k">Hospitals</div><div class="proof-v">Operational</div><div class="proof-s">ICU-style command wall for deterioration visibility and escalation routing.</div></div>
          <div class="proof-card"><div class="proof-k">Investors</div><div class="proof-v">Credible</div><div class="proof-s">Live predictive product story with platform motion in seconds.</div></div>
          <div class="proof-card"><div class="proof-k">Insurers</div><div class="proof-v">Predictive</div><div class="proof-s">Shows earlier intervention logic and lower-cost escalation potential.</div></div>
          <div class="proof-card"><div class="proof-k">Patients</div><div class="proof-v">Protective</div><div class="proof-s">Feels proactive, responsive, and clinically reassuring.</div></div>
        </div>
      </div>

      <div class="right-col">
        <div class="rail">
          <div class="panel-title">Mission control metrics</div>
          <div class="rail-grid">
            <div class="rail-card"><div class="rail-k">Engine State</div><div class="rail-v" id="r-engine">LIVE</div><div class="rail-s">Predictive scoring engine online</div></div>
            <div class="rail-card"><div class="rail-k">Focus Patient</div><div class="rail-v" id="r-focus">p101</div><div class="rail-s">Current mission-control focus</div></div>
            <div class="rail-card"><div class="rail-k">Stream Rate</div><div class="rail-v">2s</div><div class="rail-s">Command-center event cadence</div></div>
            <div class="rail-card"><div class="rail-k">Data Mode</div><div class="rail-v">AI</div><div class="rail-s">Predictive monitoring simulation</div></div>
          </div>
        </div>

        <div class="feed">
          <div class="panel-title">Live alert feed</div>
          <div class="panel-copy">Newest alerts appear first with severity, patient ID, recommended action, and AI confidence.</div>
  ...
  <script>
    function buildWavePath(severity) {
    ...
    function applyMonitor(slot, patient) {
      const status = (patient.status || "stable").toLowerCase();
      const cls = status === "critical" ? "critical" : (status === "high" ? "high" : "stable");
      ...
      document.getElementById(slot + "-hr").textContent = patient.heart_rate;
      document.getElementById(slot + "-spo2").textContent = patient.spo2;
      document.getElementById(slot + "-bp").textContent = patient.bp_systolic + "/" + patient.bp_diastolic;
      ...
      card.className = "monitor " + cls;

      const wave = document.getElementById(slot + "-wave");
      wave.setAttribute("d", buildWavePath(cls));
      wave.setAttribute("class", "wave-line " + cls);
    }
    ...
    function renderTicker(alerts) {
      ...
      ticker.innerHTML = alerts.slice(0, 6).map(function(a) {
        const sev = (a.severity || "").toLowerCase();
        return `<div class="ticker-pill"><span class="dot ${sev}"></span>${a.title} · ${a.patient_id} · Risk ${Number(a.risk_score).toFixed(1)} · ${a.recommended_action}</div>`;
      }).join("");
    }

    async function refreshOverview() {
      try {
        const r = await fetch("/api/v1/dashboard/overview?tenant_id=demo&refresh=" + Date.now(), { cache: "no-store" });
        if (!r.ok) return;
        const data = await r.json();
        document.getElementById("k-open").textContent = data.open_alerts ?? 0;
        document.getElementById("k-critical").textContent = data.critical_alerts ?? 0;
        document.getElementById("k-risk").textContent = Number(data.avg_risk_score ?? 0).toFixed(1);
        document.getElementById("k-patients").textContent = data.patients_with_alerts ?? 0;
        document.getElementById("k-events").textContent = data.events_last_hour ?? 0;
        document.getElementById("r-focus").textContent = data.focus_patient_id || "p101";
      } catch (e) {
        console.error(e);
      }
    }
    ...
    es.onmessage = function(evt) {
      ...
      if (patients[0]) applyMonitor("m1", patients[0]);
      if (patients[1]) applyMonitor("m2", patients[1]);
      if (patients[2]) applyMonitor("m3", patients[2]);
      if (patients[3]) applyMonitor("m4", patients[3]);

      renderFeed(alerts);
      renderTicker(alerts);

      document.getElementById("k-open").textContent = payload.summary?.open_alerts ?? 0;
      document.getElementById("k-critical").textContent = payload.summary?.critical_alerts ?? 0;
      document.getElementById("k-risk").textContent = Number(payload.summary?.avg_risk_score ?? 0).toFixed(1);
      document.getElementById("k-patients").textContent = payload.summary?.patients_with_alerts ?? 0;
      document.getElementById("k-events").textContent = payload.summary?.events_last_hour ?? 0;
      document.getElementById("r-focus").textContent = payload.summary?.focus_patient_id || "p101";
    };
  </script>
</body>
</html>
