...
def api_healthz():
    try:
        ...
        return jsonify({"status": "ok", "service": "early-risk-alert-api", "db_ok": True, "db_error": None}), 200
    except Exception as e:
        return jsonify({"status": "degraded", "service": "early-risk-alert-api", "db_ok": False, "db_error": str(e)}), 503
...
def command_center_summary():
    ...
    return jsonify(
        {
            "tenant_id": tenant_id,
            "open_alerts_by_severity": [{"severity": row["severity"], "count": int(row["count"])} for row in rows],
            "patient_risk_mix": [{"risk_level": row["risk_level"], "count": int(row["count"])} for row in risk_rows],
